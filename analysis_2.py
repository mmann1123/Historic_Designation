# Loading Historic Districts and Census Data
# %%
...
panel_df["tract_id"] = panel_df["GEOID"]
...
def run_did_analysis(var, pre_period=2000, min_post_period=2005):
    print(f"\nRunning DiD for {var}...")
...
for var in rent_vars + ["nonwhite_nonasian_pct"]:
    if var in panel_df.columns:
        ...
        pvalue = model.pvalues["treated_post"]
...
for tract_id in panel_df["tract_id"].unique():
    ...
    if not all(hd_status_by_year) and any(hd_status_by_year):
